...
export async function deployToVercel(
  ...
  // Pick the shortest clean .vercel.app alias (not the long hash one)
  const cleanAlias = ready.alias
    ?.filter((a: string) => a.endsWith(".vercel.app"))
    ?.sort((a: string, b: string) => a.length - b.length)[0];

  return {
    deploymentId: ready.id,
    url: `https://${cleanAlias ?? ready.url}`,
  };
}
