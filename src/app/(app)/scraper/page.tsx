...
import { TEMPLATE_CATEGORIES } from "@/lib/utils";
import {
  Search, MapPin, Phone, Mail, Globe, Star,
  Instagram, Facebook, Plus, CheckCircle2,
  Loader2, Sparkles, Building2, Users, ChevronRight,
  Image, MessageSquare, Clock,
} from "lucide-react";
...
const NICHES = [
  ...
  { value: "lawyer", label: "⚖️ Lawyers & Notaires" },
  { value: "real-estate", label: "🏠 Immobilier" },
  { value: "retail", label: "🛍️ Boutiques & Retail" },
...
export default function ScraperPage() {
  ...
  const handleScrape = async () => {
    ...
    const nicheLabel = NICHES.find(n => n.value === niche)?.label.split(" ").slice(1).join(" ") || niche;

    try {
      setProgressMsg(`Starting Apify scraper for "${nicheLabel}" in ${targetCity}...`);
      setProgress(10);
      ...
      setProgress(90);
      setProgressMsg(`Found ${data.results?.length || 0} businesses!`);
      ...
      setProgress(100);
      setProgressMsg(`Done! Found ${businesses.length} businesses.`);
  ...
  const handleAddToLeads = async () => {
    ...
    if (toAdd.length === 0) { toast.error("Select at least one business"); return; }
      ...
      try {
        ...
        if (!error) {
          ...
          setResults(prev => prev.map(r =>
            r.name === biz.name ? { ...r, added: true, selected: false } : r
          ));
        }
      } catch (e) {
        console.error("Failed to add:", biz.name);
      }
    }

    toast.success(`✅ Added ${successCount} businesses to leads!`);
    setAdding(false);
  };
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Find and scrape businesses from Google Maps with all their data"
        actions={
          selectedCount > 0 && (
            <Button
        ...
        <div className="grid grid-cols-4 gap-4 items-end">
          <div>
            ...
            <select
              ...
              className="w-full h-9 px-3 text-sm bg-white border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500"
            >
              {NICHES.map(n => (
          ...
          <div>
            ...
            {city === "custom" && (
              <input
                ...
                className="w-full h-9 px-3 text-sm bg-white border border-surface-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-500 mt-2"
              />
            )}
          </div>
          <div>
          ...
          <Button
            icon={scraping ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedCount > 0 && (
                <Button
                  ...
                  onClick={handleAddToLeads}
                  loading={adding}
                  className="bg-green-600 hover:bg-green-700 border-0 ml-2"
                >
                  Add {selectedCount} to Leads
                </Button>
              )}
          ...
          <div className="grid grid-cols-1 gap-4">
            {results.map((biz, idx) => (
              <Card
                key={idx}
                padding="none"
                className={`overflow-hidden transition-all ${biz.added ? "opacity-60" : ""} ${biz.selected && !biz.added ? "ring-2 ring-brand-400" : ""}`}
              >
                <div className="flex">
                  {/* Photo */}
                  <div className="w-40 h-36 flex-shrink-0 bg-surface-100 relative overflow-hidden">
                    {biz.photos?.[0] ? (
                    ...
                    {biz.photos?.length > 1 && (
                      <div className="absolute bottom-1 right-1 bg-black/60 text-white text-[10px] px-1.5 py-0.5 rounded flex items-center gap-0.5">
                        <Image className="w-2.5 h-2.5"/> {biz.photos.length}
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {biz.google_rating > 0 && (
                            <span className="flex items-center gap-0.5 text-[11px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 mb-2">
                          {biz.address && (
                            <span className="flex items-center gap-1 text-[11px] text-slate-500">
                              <MapPin className="w-2.5 h-2.5"/> {biz.address}
                            </span>
                          )}
                          {biz.phone && (
                            <span className="flex items-center gap-1 text-[11px] text-slate-500">
                          ...
                          {biz.email && (
                            <span className="flex items-center gap-1 text-[11px] text-slate-500">
                              <Mail className="w-2.5 h-2.5"/> {biz.email}
                            </span>
                          )}
                          {biz.website && (
                            <a href={biz.website} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[11px] text-brand-600 hover:text-brand-700">
                              <Globe className="w-2.5 h-2.5"/> Website
                            </a>
                          )}
                          {biz.instagram && (
                            <a href={biz.instagram} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[11px] text-pink-600">
                              <Instagram className="w-2.5 h-2.5"/> Instagram
                            </a>
                          )}
                          {biz.facebook && (
                            <a href={biz.facebook} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[11px] text-blue-600">
                        ...
                        {biz.description && (
                          <p className="text-[11px] text-slate-400 line-clamp-1 mb-2">{biz.description}</p>
                        )}

                        {/* Tags */}
                        <div className="flex flex-wrap gap-1">
                          {biz.photos?.length > 0 && (
                            <span className="text-[10px] bg-blue-50 border border-blue-200 text-blue-600 px-1.5 py-0.5 rounded-full">
                              {biz.photos.length} photos
                            </span>
                          )}
                          {biz.reviews?.length > 0 && (
                            <span className="text-[10px] bg-green-50 border border-green-200 text-green-600 px-1.5 py-0.5 rounded-full">
                              {biz.reviews.length} reviews
                            </span>
                          )}
                          {biz.services?.length > 0 && (
                            <span className="text-[10px] bg-purple-50 border border-purple-200 text-purple-600 px-1.5 py-0.5 rounded-full">
                              {biz.services.length} services
                            </span>
                          )}
                          {biz.opening_hours?.length > 0 && (
                            <span className="text-[10px] bg-amber-50 border border-amber-200 text-amber-600 px-1.5 py-0.5 rounded-full">
                              Hours available
                            </span>
                          )}
                          {biz.phone && (
                            <span className="text-[10px] bg-emerald-50 border border-emerald-200 text-emerald-600 px-1.5 py-0.5 rounded-full">
                              ✓ Phone
                            </span>
                          )}
                        </div>
      ...
      {!scraping && results.length === 0 && (
        <Card className="text-center py-16">
          ...
          <p className="text-xs text-slate-400 max-w-sm mx-auto">
            Select a business type and city, then click Scrape Now to find businesses with all their data — photos, reviews, contact info, social media and more.
          </p>
        </Card>
      )}
    </div>
