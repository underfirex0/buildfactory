...
export default function BuildQueuePage() {
  ...
  const [bulkProgress, setBulkProgress] = useState<null | { total: number; successful: number; failed: number }>(null);
  ...
  const handleCreateBuild = async () => {
    ...
    setBuilding(true);
    try {
      const { data: buildRecord, error: insertErr } = await supabase.from("builds").insert({ lead_id: newBuild.lead_id, template_id: newBuild.template_id, status: "building", started_at: new Date().toISOString() }).select().single();
  ...
  const handleBulkBuild = async () => {
    ...
    try {
      ...
      toast.success(`Bulk build done! ${result.summary.successful}/${result.summary.total} succeeded`);
      await fetchAll();
    } catch (err: any) {
  ...
  const statusCounts = { all: builds.length, pending: builds.filter((b) => b.status === "pending").length, building: builds.filter((b) => b.status === "building").length, done: builds.filter((b) => b.status === "done").length, failed: builds.filter((b) => b.status === "failed").length };
  ...
  return (
    <div className="animate-fade-in">
      ...
      <Card padding="none">
        {loading ? (
          ...
          <table className="data-table">
            <thead><tr><th>Business</th><th>Template</th><th>Status</th><th>Duration</th><th>Created</th><th>Actions</th></tr></thead>
            <tbody>
              {filteredBuilds.map((build) => {
                ...
                let duration = "—";
                if (build.started_at && build.completed_at) { const ms = new Date(build.completed_at).getTime() - new Date(build.started_at).getTime(); duration = `${(ms / 1000).toFixed(1)}s`; }
                return (
                  <tr key={build.id} className="group">
                    ...
                    <td><span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-1 rounded-full border ${meta.bg} ${meta.color}`}><StatusIcon status={build.status} />{meta.label}</span></td>
                    <td><span className="text-xs text-slate-400">{duration}</span></td>
                    <td><span className="text-xs text-slate-400">{formatDateRelative(build.created_at)}</span></td>
                    <td>
                      {build.status === "done" && build.output_path ? (
                        <Button size="sm" variant="secondary" icon={<Download className="w-3 h-3" />} onClick={() => handleDownload(build)}>Download</Button>
                      ) : build.status === "failed" ? (
                        <span className="text-[11px] text-red-500 truncate max-w-[140px] block" title={build.error_msg ?? ""}>{build.error_msg ?? "Unknown error"}</span>
                      ) : null}
                    </td>
                  </tr>
                );
      ...
      {/* Single Build Modal */}
      <Modal isOpen={newBuildOpen} onClose={() => setNewBuildOpen(false)} title="Build Website" footer={<><Button variant="secondary" onClick={() => setNewBuildOpen(false)}>Cancel</Button><Button onClick={handleCreateBuild} loading={building} icon={<Hammer className="w-3.5 h-3.5" />}>Start Build</Button></>}>
        <div className="space-y-4">
          <div className="bg-brand-50 border border-brand-100 rounded-xl p-4"><p className="text-xs font-semibold text-brand-700 mb-1">How it works</p><p className="text-xs text-brand-600 leading-relaxed">Select a lead and a matching template. BuildFactory will replace all placeholders with the lead's data and generate a downloadable ZIP.</p></div>
      ...
      {/* Bulk Build Modal */}
      <Modal isOpen={bulkOpen} onClose={() => { setBulkOpen(false); setBulkProgress(null); setBulkConfig({ template_id: "", lead_ids: [] }); }} title="Bulk Build Websites" size="lg"
        footer={<><Button variant="secondary" onClick={() => setBulkOpen(false)}>Close</Button><Button onClick={handleBulkBuild} loading={bulkBuilding} icon={<Layers className="w-3.5 h-3.5" />} disabled={!bulkConfig.template_id || bulkConfig.lead_ids.length === 0}>Build {bulkConfig.lead_ids.length > 0 ? `${bulkConfig.lead_ids.length} Sites` : "All Sites"}</Button></>}
      >
        <div className="space-y-5">
          {bulkProgress && (
            <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4">
              <p className="text-sm font-semibold text-emerald-700 mb-2">✅ Bulk Build Complete!</p>
              <div className="grid grid-cols-3 gap-3 text-center">
                ...
                <div className="bg-white rounded-lg p-2 border border-emerald-100"><p className="text-lg font-bold text-emerald-600">{bulkProgress.successful}</p><p className="text-[11px] text-slate-500">Successful</p></div>
          ...
          {bulkConfig.lead_ids.length > 0 && bulkConfig.template_id && !bulkBuilding && !bulkProgress && (
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 flex items-start gap-2">
              ...
              <p className="text-xs text-amber-700">This will generate <strong>{bulkConfig.lead_ids.length}</strong> website packages in parallel. This may take a minute.</p>
            </div>
          )}
        </div>
      </Modal>
